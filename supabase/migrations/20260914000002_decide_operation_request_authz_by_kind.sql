-- ════════════════════════════════════════════════════════════════════════════
-- Fase 3E-B1 — autorización por KIND, no una sola para todos
-- ════════════════════════════════════════════════════════════════════════════
--
-- Hasta acá la RPC autorizaba con un único permiso, sin importar qué tipo de
-- solicitud fuera:
--
--     owner OR can_confirm_reservations
--
-- Eso significaba que el permiso llamado "Confirmar reservas" también habilitaba
-- gestionar consultas inmobiliarias. Brecha semántica, no decisión de producto.
--
-- ── EL MAPA NUEVO ───────────────────────────────────────────────────────────
--
--   kind                  permiso requerido (además de owner, que siempre puede)
--   ─────────────────────────────────────────────────────────────────────────
--   inquiry               can_manage_inquiries        ← nuevo
--   reservation_request   can_confirm_reservations
--   visit_request         can_confirm_reservations    ← legacy, ver abajo
--   table_request         can_confirm_reservations    ← legacy (gastronomía)
--   order_request         can_confirm_reservations    ← legacy (gastronomía)
--
-- `inquiry` cubre exactamente property_inquiry, monthly_rental_inquiry y
-- general_inquiry, según el mapeo de confirm_submission_and_create_operation.
-- general_inquiry es de food_service y esta fase no lo trabaja, pero comparte
-- kind: hereda el permiso nuevo por construcción. Es coherente —también es una
-- consulta— y se deja anotado para que 3E-C no se sorprenda.
--
-- ── POR QUÉ visit_request SIGUE CON EL PERMISO DE RESERVAS ──────────────────
--
-- Deliberado y explícitamente temporal. Corregir la autorización global obliga
-- a decir algo sobre visit_request; la alternativa a mantener su comportamiento
-- actual sería inventar can_manage_visits, y las visitas son 3E-B2, donde
-- todavía hay seis decisiones de producto abiertas (¿la empresa fija una hora?
-- ¿se asigna asesor? ¿qué ciclo de vida?). Elegir su permiso ahora sería
-- adelantarse a esas decisiones.
--
-- Así que visit_request conserva can_confirm_reservations como comportamiento
-- LEGACY DOCUMENTADO, no como solución. Su migración de permiso pertenece a
-- 3E-B2 y queda anotada acá para que no se pierda.
--
-- ── QUÉ DEVUELVE AL DENEGAR ─────────────────────────────────────────────────
--
-- El outcome sigue siendo 'forbidden' —los callers y los tests existentes no
-- cambian— pero ahora acompaña `kind` y `required_permission`. Es aditivo, y le
-- permite a la UI decir qué permiso falta en vez de un "no tenés permiso" seco.
-- No filtra nada: el usuario ya puede ver sus propios permisos.
--
-- ── LO QUE YA HACÍA (3E-A.2, cierre) ────────────────────────────────────────
--
-- El último riesgo abierto del reporte de 3E-A.2: una solicitud vieja aprobada
-- meses después creaba una pre-reserva que arranca en el pasado. La IA lo
-- bloquea desde siempre; este camino no.
--
-- ── POR QUÉ NO VA EN LA FUNCIÓN CANÓNICA (§3) ───────────────────────────────
--
-- check_temporary_rental_eligibility NO gana esta regla, a propósito. Las
-- reglas que viven ahí son propiedades de la PROPIEDAD (estadía mínima,
-- capacidad, estado comercial): valen para cualquiera que escriba. Esta
-- depende del ACTOR:
--
--   · MANUAL (createReservationAction) — SÍ puede registrar una reserva cuyo
--     inicio ya pasó. Es carga retroactiva de datos, una función legítima del
--     dashboard, y sigue intacta.
--   · FORMULARIO → APROBACIÓN — NO puede. Una pre-reserva nueva que arranca
--     ayer no representa nada real: nadie va a ocupar una fecha que pasó, y el
--     hold de 24 h sobre fechas vencidas es basura.
--   · IA — sigue bloqueándolo como siempre, con su propio chequeo en
--     TypeScript, sin cambios.
--
-- Meterla en el core le quitaría al dashboard la carga retroactiva. Es
-- exactamente el tipo de regla que el reporte de 3E-A.2 clasificó como Tipo B
-- por depender del actor, y acá se implementa en el único flujo donde aplica.
--
-- ── QUÉ SIGNIFICA "HOY" (§1) ────────────────────────────────────────────────
--
-- Auditoría de la semántica temporal del proyecto:
--
--   · `tenants.timezone` EXISTE (TEXT NOT NULL DEFAULT
--     'America/Argentina/Buenos_Aires'), pero su propio COMMENT lo dice
--     textualmente: "Ningún código actual la consume todavía — metadata
--     capturada para uso futuro". No hay noción de timezone por tenant en
--     vigencia; leerla acá sería estrenar una semántica que ningún otro flujo
--     respeta, y produciría dos definiciones de "hoy" en el mismo producto.
--
--   · TODO el resto del código define hoy igual: `new Date()
--     .toISOString().split('T')[0]`, o sea la fecha UTC. Así lo hacen los dos
--     tools de la IA, completeReservationAction, los vencimientos de alquileres
--     mensuales y los badges del dashboard. Es la semántica vigente.
--
--   · En SQL, monthly_rental_payments usa CURRENT_DATE. La base corre con
--     TimeZone = UTC, así que CURRENT_DATE coincide con la fecha UTC —
--     verificado en el proyecto: ambas dan el mismo día.
--
-- Se usa `(NOW() AT TIME ZONE 'UTC')::DATE`, no CURRENT_DATE: da exactamente el
-- mismo resultado hoy, pero no depende del TimeZone de la sesión, que un caller
-- podría cambiar. La comparación es `< hoy` (estricta), igual que la de la IA
-- (`startStr < todayStr`): una solicitud cuyo check-in es HOY sigue siendo
-- aprobable.
--
-- Si algún día el producto consume tenants.timezone, este es uno de los lugares
-- a revisar — junto con los otros ~15 sitios que hoy usan la fecha UTC.
--
-- ── LO QUE YA HACÍA (3E-A.2) ────────────────────────────────────────────────
--
...
--     → fechas coherentes
--     → check-in no pasado   ← nuevo en este cierre
--     → elegibilidad canónica
--     → disponibilidad
--     → pricing
--     → INSERT reservation
...
--
-- El chequeo de fecha pasada va primero entre las validaciones de negocio: si
-- la solicitud está vencida, el motivo más útil para el asesor es ese, no que
-- además supere la capacidad.
--
...
BEGIN
  ...
  -- ── Autorización según el KIND real de la solicitud ─────────────────────
  -- Se resuelve DESPUÉS de leer la operación (ya está lockeada arriba), porque
  -- el permiso necesario depende de qué tipo de solicitud es.
  v_needs := CASE v_op.kind
    WHEN 'inquiry' THEN 'can_manage_inquiries'
    ELSE 'can_confirm_reservations'   -- reservation_request, visit_request (legacy), table_request, order_request
  END;

  v_allowed := v_member.role = 'owner'                                  -- el owner siempre puede
  ...
  THEN
    ...
    -- ── Regla propia de este flujo: el check-in no puede haber pasado ──────
    -- Solo acá. La creación manual conserva la carga retroactiva; la IA tiene
    -- su propio chequeo equivalente en TypeScript; la función canónica de
    -- elegibilidad NO conoce esta regla. Ver el encabezado.
    --
    -- Fecha UTC explícita (no CURRENT_DATE, que depende del TimeZone de la
    -- sesión) y comparación estricta: una solicitud cuyo check-in es HOY se
    -- puede aprobar.
    IF v_start < (NOW() AT TIME ZONE 'UTC')::DATE THEN
      RETURN jsonb_build_object(
        'outcome',        'past_start_date',
        'requested_date', v_start,
