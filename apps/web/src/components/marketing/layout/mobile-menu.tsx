        ...
        ? createPortal(
            <div className="fixed inset-0 z-50 flex flex-col bg-white">
              ...
              <nav className="flex flex-col gap-1 overflow-y-auto px-4 py-6">
                ...
                {/* next/link's plain Link, not the locale-aware one from @/i18n/navigation: /login must never get a locale prefix. */}
                <Link
                  href="/login"
                  onClick={() => setOpen(false)}
                  className="mt-2 rounded-lg border-t border-slate-100 px-3 py-3 pt-4 text-base font-semibold text-brand-green hover:bg-slate-50"
                >
                  {loginLabel}
                </Link>
              </nav>
