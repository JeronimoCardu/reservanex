...
import { getBusinessSettingsAction }  from '@/actions/tenant-settings'
import { BusinessSettingsForm }       from './business-settings-form'
import { cn }                         from '@/lib/utils'

export const metadata: Metadata = { title: 'Negocio — Configuración — ReservaNex' }
  ...
  return (
    <div className="flex flex-col overflow-hidden h-full">
          ...
          {TABS.filter(t => !t.ownerOnly || ctx.role === 'owner').map(t => (
            <Link
              ...
              className={cn(
                ...
                t.href === '/dashboard/settings/business'
      ...
      <div className="border-b bg-background px-6 py-4">
        <h1 className="text-lg font-semibold">Negocio</h1>
        <p className="text-xs text-muted-foreground">
          {readOnly
            ? 'Información de contacto y horarios del negocio.'
            : 'Datos de contacto, redes sociales y horarios del negocio.'}
        </p>
      </div>

      <div className="flex-1 overflow-y-auto px-6 py-6">
        {settings ? (
          <BusinessSettingsForm settings={settings} readOnly={readOnly} />
        ) : (
